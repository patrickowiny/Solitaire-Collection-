...
export class GamePresenter extends GamePresenterBase<IGame> {
    ...
    constructor(game: IGame, rootView: IView) {
        ...
        const wastePiles = [game.waste0, game.waste1, game.waste2];
        for (const waste of wastePiles) {
            const pileView = this.createPileView_(waste);
            pileView.showFrame = true;
            pileView.zIndex = 50;
    ...
    private layoutPiles_() {
        ...
        // Row 1: Stock + Waste x3 + Foundations x4
        {
        ...
        const wastePiles = [this.game_.waste0, this.game_.waste1, this.game_.waste2];
        for (let i = 0; i < wastePiles.length; ++i) {
            const pile = wastePiles[i] ?? error();
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(i + 1), yTop);
        }

        for (let i = 0; i < this.game_.foundations.length; ++i) {
            const pile = this.game_.foundations[i] ?? error();
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(i + 4), yTop);
        }
