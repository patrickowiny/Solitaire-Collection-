...
export class Game extends GameBase implements IGame {
    ...
    protected *restart_(rng: prand.RandomGenerator) {
        ...
        // Deal 52 cards: 6 cards to the topmost pile of each wing (0 and 5), 5 cards to others (1..4 and 6..9)
        const targets = [6, 5, 5, 5, 5, 6, 5, 5, 5, 5];
        for (let i = 0; i < 10; ++i) {
            const target = targets[i] ?? 5;
            const destTableau = this.tableaux[i] ?? Debug.error();
    ...
    private isTableauxDrop_(card: Card, pile: Pile): boolean {
        ...
        if (topCard) {
            ...
            const valTop = this.getCardValue_(topCard);
            const valCard = this.getCardValue_(card);
            return valCard === valTop + 1 || valCard === valTop - 1;
        } else {
            return true; // Empty tableau can receive any single card
        }
    }
