...
export class GamePresenter extends GamePresenterBase<IGame> {
    ...
    private updateSizes_() {
        let { sizeX, sizeY } = this.calculateCardSize(7, margin);
        if (sizeY > 17) {
            sizeY = 17;
            this.sizeX = sizeY / 1.555555555555;
        } else {
            this.sizeX = sizeX;
        }
        this.sizeY = sizeY;
    ...
    private layoutPiles_() {
        ...
        // Position helper for 5 tableau rows (0 to 4), centered at y = 0
        const yPos = (rowIdx: number) => {
            return (rowIdx - 2) * (this.sizeY + scaledMarginY);
        };

        // Position helper for 4 foundation rows (0 to 3), centered at y = 0
        const foundationYPos = (rowIdx: number) => {
        ...
        for (let i = 0; i < 4; ++i) {
            ...
            pileView.rect = new Rect(this.sizeX, this.sizeY, foundationX, foundationYPos(i));
        }

        // Apply layouts to left tableaux (indices 0 to 4) fanned to the left (negative fanXUp)
        for (let i = 0; i < 5; ++i) {
        ...
        // Apply layouts to right tableaux (indices 5 to 9) fanned to the right (positive fanXUp)
        for (let i = 0; i < 5; ++i) {
            const pile = this.game_.tableaux[i + 5] ?? error();
            const pileView = this.getPileView_(pile);
