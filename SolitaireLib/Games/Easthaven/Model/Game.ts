...
export class Game extends GameBase implements IGame {
    ...
    protected *restart_(rng: prand.RandomGenerator) {
        ...
        // Deal 7 tableau piles of 3 cards each (bottom 2 face down, top face up)
        for (let i = 0; i < TABLEAUX_COUNT; ++i) {
    ...
    private hasAvailableKingMove_(): boolean {
        const hasEmptyTableau = this.tableaux.some(tp => tp.length === 0);
        if (!hasEmptyTableau) return false;

        for (const tableau of this.tableaux) {
            for (const card of tableau) {
                if (card.rank === Rank.King && card.faceUp && card.pileIndex > 0) {
