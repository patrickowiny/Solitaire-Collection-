...
export class GamePresenter extends GamePresenterBase<IGame> {
    ...
    constructor(game: IGame, rootView: IView) {
        ...
        // Create foundation piles (7 to 10)
    ...
    private updateSizes_() {
        // Spiderette has 7 tableau columns exactly as in Klondike.
    ...
    private layoutPiles_() {
        ...
        // Layout following Spider / Klondike logic:
        // Top Row: stock is placed at the far left (column 0).
        {
        ...
        // Top Row: foundations are placed at columns 3 to 6 on the right.
        for (let i = 0; i < this.game_.foundations.length; ++i) {
            const pile = this.game_.foundations[i] ?? error();
            const pileView = this.getPileView_(pile);
            // Put foundations in the last 4 columns (columns 3, 4, 5, 6)
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(tableSize - this.game_.foundations.length + i), topY);
        }

        // Bottom Row: tableaux (columns 0 to 6)
        for (let i = 0; i < this.game_.tableaux.length; ++i) {
            const pile = this.game_.tableaux[i] ?? error();
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(i), bottomY);
            // Columns can grow quite long, so we use appropriate cascading spacing
