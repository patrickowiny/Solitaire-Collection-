...
export class GamePresenter extends GamePresenterBase<IGame> {
    ...
    private readonly cellPiles_: PileView[] = [];
    ...
    constructor(game: IGame, rootView: IView) {
        ...
        // Cells 8-15:
        for (let i = 0; i < this.game_.cells.length; ++i) {
            const pileView = this.createPileView_(game.cells[i] ?? error());
            pileView.showFrame = true;
            pileView.zIndex = 800;
            this.cellPiles_.push(pileView);
        }
    ...
    private updateSizes_() {
        const { sizeX, sizeY } = this.calculateCardSize(12, margin);
        this.sizeX = sizeX;
        this.sizeY = sizeY;
    }
    ...
    private layoutPiles_() {
        ...
        // Row 1 (Top): Cells on left side (columns 0-7), Foundations on right side (columns 8-11)
        for (let i = 0; i < this.game_.cells.length; ++i) {
            const pile = this.game_.cells[i] ?? error();
        ...
        for (let i = 0; i < this.game_.foundations.length; ++i) {
            ...
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(i + 8), topY);
        }

        // Row 2 (Bottom): Tableau stacks 0-7 arranged horizontally directly beneath, centered (columns 2-9)
        for (let i = 0; i < this.game_.tableaux.length; ++i) {
            const pile = this.game_.tableaux[i] ?? error();
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(i + 2), bottomY);
