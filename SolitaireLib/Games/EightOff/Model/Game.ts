...
export class Game extends GameBase implements IGame {
    ...
    constructor(options: GameOptions) {
        ...
        for (let i = 0; i < 8; ++i) {
            const pile = new Pile(this);
            this.cells.push(pile);
            this.piles.push(pile);
        }

        // Foundation piles 16 to 19
        for (let i = 0; i < 4; ++i) {
            const pile = new Pile(this);
    ...
    protected *restart_(rng: prand.RandomGenerator) {
        ...
        // Deal the remaining 4 cards, one each, into 4 of the 8 cells.
        for (let i = 0; i < 4; ++i) {
            const destCell = this.cells[i] ?? Debug.error();
            const card = tempPile.peek();
            if (card) {
                destCell.push(card);
                card.faceUp = true;
    ...
    protected canDrag_(card: Card): { canDrag: boolean; extraCards: Card[] } {
        ...
        if (this.cells.indexOf(card.pile) >= 0) {
    ...
    protected previewDrop_(card: Card, pile: Pile): boolean {
        return this.isTableauDrop_(card, pile) || this.isFoundationDrop_(card, pile) || this.isCellDrop_(card, pile);
    }
    ...
    private isCellDrop_(card: Card, pile: Pile): boolean {
        if (card.pile === pile) return false;
        if (this.cells.indexOf(pile) < 0) return false;
    ...
    private isTableauDrop_(card: Card, pile: Pile): boolean {
        ...
        let emptyCells = 0;
        for (const fc of this.cells) {
            if (fc.length === 0) emptyCells++;
        }
        ...
        const maxMove = emptyCells + emptyTableaux + 1;
    ...
    private *doAutoMoves_() {
        mainLoop: while (true) {
            if (this.options.autoMoveToFoundation > 0) {
                ...
                const sources = [...this.tableaux, ...this.cells];
