...
export class GamePresenter extends GamePresenterBase<IGame> {
    ...
    private updateSizes_() {
        // Lay out in 10 columns (Stock, Waste, 8 foundations)
    ...
    private layoutPiles_() {
        ...
        // Stock at column 0 (Top)
        {
            const pile = this.game_.stock;
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(0), yTop);
        }
        // Waste at column 1 (Top)
        {
            const pile = this.game_.waste;
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(1), yTop);
        }
        // Foundations at columns 2 to 9 (Top)
        for (let i = 0; i < this.game_.foundations.length; ++i) {
            const pile = this.game_.foundations[i] ?? error();
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(2 + i), yTop);
        }

        // Reserve at column 0 (Bottom)
        {
            const pile = this.game_.reserve;
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(0), yBottom);
        }
        // Tableaux at columns 2 to 9 (Bottom)
        for (let i = 0; i < this.game_.tableaux.length; ++i) {
            const pile = this.game_.tableaux[i] ?? error();
            const pileView = this.getPileView_(pile);
            pileView.rect = new Rect(this.sizeX, this.sizeY, xPos(2 + i), yBottom);
